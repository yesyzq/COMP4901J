...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        ...
        prev_h = next_h
        h[:,i,:] = prev_h  # eye: this should be executed after prev_h = next_h
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return h, cache
...
def rnn_backward(dh, cache):
    ...
    for i in range(T - 1, -1, -1):
        dh_curr = dh_prev + dh[:,i,:]  # eye: update dh_curr first before backprop
        dx[:,i,:], dh_prev, dWx_, dWh_, db_ = rnn_step_backward(dh_curr, cache[i])
        if i is 0:
            dh0 = dh_prev
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (i, f, o, g, prev_c, next_c, W, X, x.shape[1])
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
